...
def test_create_recipes(testing_client):
    ...
    THEN check the response is valid
    """
    response = testing_client.post('/recipes', json={'name': 'test', 'rate': '3', 'favorite': 'false'})
    assert response.status_code == 200
...
def test_update_recipes(testing_client):
    ...
    THEN check the response is valid
    """
    response = testing_client.put('/recipes/1', json={'name': 'test', 'rate': '3', 'favorite': 'false'})
    assert response.status_code == 200
...
def test_main_post(testing_client):
    ...
    """
    response = testing_client.post('/', json={'name': 'test', 'rate': '3', 'favorite': 'false'})
